...
const Board = () => {
  ...
  // Local state only toggles the visibility layout frame
  const [isAdding, setIsAdding] = useState(false);

  const { register, handleSubmit, reset, formState: { errors } } = useForm();

  const onSubmitList = (data) => {
    dispatch(addList({ title: data.listTitle.trim() }));
    reset(); 
    setIsAdding(false);
  };
      ...
      <div className="flex items-start gap-4 overflow-x-auto pb-4">
        ...
        <div className="w-72 shrink-0 bg-white/10 p-3 rounded-xl transition-all">
          {!isAdding ? (
            ...
            <form onSubmit={handleSubmit(onSubmitList)} className="flex flex-col gap-2">
              <input
                type="text"
                placeholder="Enter list title..."
                className={`w-full p-2 bg-white rounded-lg border text-sm text-gray-900 focus:outline-none ${
                  errors.listTitle ? 'border-red-500 focus:border-red-500' : 'border-solid border-gray-300 focus:border-blue-500'
                }`}
                autoFocus
                {...register("listTitle", { required: "List title is required" })}
              />
              
              {errors.listTitle && (
                <span className="text-red-400 text-xs px-1">{errors.listTitle.message}</span>
              )}
